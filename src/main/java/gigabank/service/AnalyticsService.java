...
/*    public Map<String, BigDecimal> getMonthlySpendingByCategories(User user, Set<String> categories) {
        ...
        Set<String> validatedCategories = categories.stream()
                .filter(TransactionService.TRANSACTION_CATEGORIES::contains)
...
/*    public PriorityQueue<Transaction> getTopNLargestTransactions(User user, int n) {
    ...
    public List<String> analyzePerformance(List<Transaction> transactions) {
        ...
        String resultTimeStream = "Фильтруем " + (transactions.size() - 1) +
        ...
        String resultTimeParallelStream = "Получено " + (transactionsFilteredAmountPar.size() - 1) + " транзакций |" +
     ...
     * Сумма LinkedList         |   7:9 мс  |   56:201 мс | 1246:914 мс
     * ===================================================================
     * Выводы:
     ...
     * 4) При нахождении суммы элементов паралелльный стрим у LinkedList,
